...
for (i in 1:1000) {
  ...
  result <- (sum(final_sample_frame3$Sum)/(50*4)) * 6 *4 * 50
...
gg3W<-ggplot(count_df3W, aes(x = Sum_Average, y = Count)) +
  ...
  labs(title = "Stratified sampling3 with replacement from 50 plots",
       x = "Predict_Total",
       y = "Count") +
...
for (i in 1:1000) {
  ...
  final_sample_frame3 <- do.call(rbind, sample_frame_list)
  final_sample_frame3$Sum <- final_sample_frame3$RandomrowL1 + final_sample_frame3$RandomrowR1+final_sample_frame3$RandomrowL2 + final_sample_frame3$RandomrowR2
  result <- (sum(final_sample_frame3$Sum)/(4*50)) * 6 * 4 * 50
...
gg3Wo<-ggplot(count_df3Wo, aes(x = Sum_Average, y = Count)) +
  ...
  labs(title = "Stratified sampling3 without replacement from 50 plots",
       x = "Predict_Total",
       y = "Count") +
  scale_linetype_manual(name = "Legend", values = c("Converge Mean" = "dashed")) +
  theme_minimal()
...
Summary_form <- data.frame(
  Condition = c('Stra3W', 'Stra3Wo'),
  Total     = c(Stra3W_converge_mean, Stra3Wo_converge_mean),
  UpperBound= c(Stra3W_upper_bound, Stra3Wo_upper_bound),
  LowerBound= c(Stra3W_lower_bound,Stra3Wo_lower_bound),
  MarginError = c(margin_of_error3W, margin_of_error3Wo),
  Std         = c(Stra3W_std,Stra3Wo_std)
